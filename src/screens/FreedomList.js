...
import { StyleSheet, Text, View, StatusBar, ListView,AsyncStorage,FlatList,List,ListItem } from 'react-native';
...
export default class App extends React.Component {
  ...
  { 
    try{
        ...
        Firebase.database().ref('Users/'+this.state.myKey+'/freedom').on('child_added', function (data) {
  ...
  addRow(data) {
    console.log('Users/'+this.state.myKey+'/freedom');
    var key = Firebase.database().ref('Users/'+this.state.myKey+'/freedom').push().key
    Firebase.database().ref('Users/'+this.state.myKey+'/freedom').child(key).set({ name: data })
    this.getItem();
  }

  async deleteRow(secId, rowId, rowMap, data) {

    await Firebase.database().ref('Users/'+this.state.myKey+'/freedom' + data.key).set(null)
 ...
 render() {
    return (
      <Container style={styles.container}>
        ...
        <Content>
          <View>
            <Text style={{fontSize:32,color :'#333333', textAlign : "center"}}>Freedom</Text>
          </View>
          <View>
